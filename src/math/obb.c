...
OBB OBB_rotate(OBB obb, float angle, V axis) {
    ...
    // The half_extents represent the half lengths along the local axes and remain unchanged.
    ...
    for (int i = 0; i < 3; i++) {
        ...
        V new_axis = V_add(V_add(V_mul(obb.axis[i], cosf(angle)),
                                 V_mul(cross_val, -sinf(angle))),
...
int OBB_intersects(OBB obb1, OBB obb2) {
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            ...
            absR[i][j] = f_abs(R[i][j]) + 1e-6f;  // Add small epsilon to counteract arithmetic errors.
        }
    }

    // Compute translation vector t from obb1.center to obb2.center and express it in obb1's coordinate frame.
    V t_vec = V_sub(obb2.center, obb1.center);
    float t[3] = { V_dot(t_vec, obb1.axis[0]),
                   V_dot(t_vec, obb1.axis[1]),
