...
typedef struct {
  V center;
  V half_extents;
  V axis[3]; // Local axes: typically the normalized right, up, and forward
             // directions.
} OBB;

// Create a new OBB given its center, half extents, and orientation axes.
// The 'axes' array should contain 3 normalized vectors representing the local
// x, y, and z axes.
OBB OBB_new(V center, V half_extents, V axes[3]);

// Create a new symmetric OBB centered at the origin with given half extents and
// orientation axes.
OBB OBB_new_sym(V half_extents, V axes[3]);

// Translate the OBB by the vector 'v' (only the center moves; orientation
// remains unchanged).
