...
ComponentCollider *ComponentCollider_new_ptr(
  ...
  component_collider->base =
      Component_new(ComponentTypeCollider, entity_id, NULL);
...
OBB ComponentCollider_get_obb(ComponentCollider *component_collider) {
  ...
  // Here we assume 'rot' is a float (in radians) in ComponentPosition and we
...
void ComponentCollider_on_collide(ComponentCollider *component_collider,
  ...
  component_collider->_on_collide_func(entity, hit_entity);
}