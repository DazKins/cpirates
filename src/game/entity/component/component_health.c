...
void ComponentHealth_tick(Component *component) {
    ...
    if (health->health <= 0 && !health->_marked_for_death) {
        Log("Entity died due to health reaching 0");
        health->_marked_for_death = 1;
    }

    // If marked for death, delete in next frame
    if (health->_marked_for_death) {
...
ComponentHealth *ComponentHealth_new_ptr(Id entity_id, int max_health) {
    ...
    component_health->_marked_for_death = 0;
