...
void HealthBarRenderer_render(HealthBarRenderer *renderer, Entity *entity, RenderContext *render_context) {
    // Only render health bars for entities with both health and position components
    ComponentHealth *health = (ComponentHealth *)Entity_get_component(entity, ComponentTypeHealth);
    ComponentPosition *position = (ComponentPosition *)Entity_get_component(entity, ComponentTypePosition);
